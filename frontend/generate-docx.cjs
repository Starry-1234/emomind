  ...
  sections: [
    {
      ...
      children: [
        ...
        h1("一、项目概述"),
        body(
          "本项目是一套面向心理健康服务的全栈 Web 应用，集成 AI 心理医生对话、心理测评报告生成、用户与管理员双端管理等功能。系统采用前后端分离架构，前端基于 React 19 + TypeScript 构建，后端基于 FastAPI + PostgreSQL，并通过 Dify 平台接入大模型能力。",
        ),
        ...
        makeTable(
          ...
          [
            ...
            [
              "API 类型生成",
              "openapi-ts 0.73（根据后端 OpenAPI 规范自动生成前端类型）",
            ],
            ["包管理", "npm / Node.js 22+"],
          ],
        ),
        ...
        makeTable(
          ...
          [
            [
              "用户认证与授权",
              "PyJWT 2.8 + passlib（Argon2 / bcrypt 密码哈希）",
            ],
        ...
        h1("五、系统架构特点"),
        bullet(
          "前后端完全分离，通过 OpenAPI 规范自动生成前端类型，保障类型安全；",
        ),
        bullet("采用 TanStack Router 文件路由，路由即文件，开发体验优秀；"),
        bullet(
          "使用 SQLModel 作为 ORM，兼具 Pydantic 的数据校验能力与 SQLAlchemy 的灵活查询；",
        ),
        bullet(
          "集成 Dify 大模型平台，实现 AI 心理医生对话与测评报告智能生成；",
        ),
        bullet(
          "前后端均支持 Docker 容器化部署，便于在 openEuler 等国产系统上运行；",
        ),
        bullet(
          "代码规范严格执行（Ruff / Biome），并通过 Playwright 实现端到端自动化测试。",
        ),
